...
has should_render_shadow => (is => 'rw', required => 1, isa => Bool, default => 0);
...
sub change_to {
    ...
    if ($new_state ne 'default' && $self->should_show_shadow(@{$self->xy})) {
        $self->should_render_shadow(1);
        $self->shadow->change_to($new_state);
    } else {
        $self->should_render_shadow(0);
    }
}
...
sub render_shadow {
    my ($self, $surface) = @_;
    $self->shadow->render($surface) if $self->should_render_shadow;
}
